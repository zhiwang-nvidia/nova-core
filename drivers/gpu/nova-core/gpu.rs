...
pub(crate) struct Gpu<'gpu> {
    ...
    ///
    /// Declared last so it is dropped last: `_gsp_irq` holds a vector from it.
    #[pin]
...
impl<'gpu> Gpu<'gpu> {
    pub(crate) fn new<'a>(
        ...
        try_pin_init!(Self {
            ...
            // above and pinned. The registration is stored in `Gpu` and never leaked, so its
            // `Drop` always runs, and field drop order runs it before the queue is freed.
            _gsp_irq <- unsafe {
                GspIrq::new(
                    pdev,
                    vectors_ref,
