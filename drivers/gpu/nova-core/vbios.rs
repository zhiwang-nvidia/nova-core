...
use crate::firmware::{FalconUCodeDescV2, FalconUCodeDescV3, FalconUCodeDesc};
...
impl FwSecBiosImage {
    ...
    pub(crate) fn header(&self, dev: &device::Device) -> Result<FalconUCodeDesc> {
        ...
        let hdr_size = match ver {
            ...
            _ => {
                dev_err!(dev, "unsupported fwsec version {}\n", ver);
                return Err(EINVAL);
            }
        };
    ...
    pub(crate) fn ucode(&self, dev: &device::Device, desc: &FalconUCodeDesc) -> Result<&[u8]> {
        let falcon_ucode_offset = self.falcon_ucode_offset;
