...
impl From<Bar0WindowTarget> for kernel::num::Bounded<u32, 2> {
    fn from(value: Bar0WindowTarget) -> Self {
        match value {
            Bar0WindowTarget::Vram => kernel::num::Bounded::<u32, 2>::new::<0>(),
        }
    }
}

impl TryFrom<kernel::num::Bounded<u32, 2>> for Bar0WindowTarget {
    type Error = Error;

    fn try_from(value: kernel::num::Bounded<u32, 2>) -> Result<Self> {
        match value.get() {
            0 => Ok(Self::Vram),
            _ => Err(EINVAL),
        }
...
impl Pramin {
    ...
    fn read_window_base(bar: &Bar0) -> u64 {
        let reg: regs::NV_PBUS_BAR0_WINDOW = bar.read(regs::NV_PBUS_BAR0_WINDOW);
...
impl PraminWindow<'_> {
    ...
    fn write_window_base(bar: &Bar0, base: u64) -> Result {
        let val = regs::NV_PBUS_BAR0_WINDOW::from_raw(0)
            .with_target(Bar0WindowTarget::Vram)
            .try_with_window_base((base >> 16) as u32)?;
        bar.write_reg(val);
        Ok(())
    }
