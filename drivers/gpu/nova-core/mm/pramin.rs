...
//! ) -> Result<()> {
//!     let pramin = KBox::pin_init(
//!         pramin::Pramin::new(bar, chipset, vram_region)?,
//!         GFP_KERNEL,
//!     )?;
...
impl<'gpu> Pramin<'gpu> {
    ...
    pub(crate) fn new(
        ...
        let current_base = regs::pramin_window_read_base(chipset.arch(), bar);
    ...
    pub(crate) fn get_window(&self) -> Result<PraminWindow<'_, 'gpu>> {
...
/// internal `MutexGuard`).
pub(crate) struct PraminWindow<'lock, 'gpu> {
    bar: Bar0<'gpu>,
    chipset: Chipset,
    vram_region: Range<VramAddress>,
    state: MutexGuard<'lock, VramAddress>,
}

impl PraminWindow<'_, '_> {
