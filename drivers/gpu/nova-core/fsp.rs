...
/// NVDM (NVIDIA Vendor Defined Message) COT (Chain of Trust) payload structure.
...
impl<'a> FmcBootArgs<'a> {
    ...
    pub(crate) fn new(
        ...
        kernel::io_write!(fmc_boot_params, .boot_gsp_rm_params.target,
            GSP_DMA_TARGET_COHERENT_SYSTEM);
        kernel::io_write!(fmc_boot_params, .boot_gsp_rm_params.gsp_rm_desc_offset, wpr_meta_addr);
        ...
        kernel::io_write!(fmc_boot_params, .gsp_rm_params.target,
