...
/// Magic value for GspFmcBootParams header (' ' << 24 | 'C' << 16 | 'M' << 8 | 'F').
...
impl<'a> FmcBootArgs<'a> {
    ...
    pub(crate) fn new(
        ...
        fmc_boot_params.magic = GSP_FMC_BOOT_PARAMS_MAGIC;
        fmc_boot_params.size =
            u16::try_from(core::mem::size_of::<GspFmcBootParams>()).map_err(|_| EINVAL)?;
