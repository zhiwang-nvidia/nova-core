...
impl Gsp {
    ...
    /// posted messages in the GSP-to-CPU queue, so the event interrupt handler tests `swgen0()`
    /// on the returned status to decide whether to drain that queue. The interrupt tree routes
    /// every falcon cause to a single vector, so a handler that finds SWGEN0 clear inspects the
    /// rest of the status to identify the cause that fired.
    ...
    ///
    /// A masked cause leaves the falcon's enabled set, so [`Self::retrigger_intr`] cannot raise
    /// it again. A handler that cannot service a cause must call this before retriggering.
    ...
    ///
    /// The falcon signals the tree on a transition of its enabled causes, so acknowledging the
    /// tree leaf while a cause is still latched leaves no transition and no further vector. A
    /// handler calls this on every path that services the vector.
    ///
