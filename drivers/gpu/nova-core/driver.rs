...
impl pci::Driver for NovaCoreDriver {
    ...
    fn probe<'bound>(
        ...
        pin_init::pin_init_scope(move || {
            ...
            // Allocate the interrupt vectors once, for the one subtree nova-core services. The
            // returned vector is shared by the self-test (if enabled), whose handler is freed
            // before it returns, and the permanent GSP handler below.
            let vector = crate::irq::alloc_vectors(pdev, crate::irq::gsp::GSP_SUBTREE)?;
            ...
            Ok(try_pin_init!(NovaCore {
                ...
                _: {
                    ...
                    crate::irq::gsp::enable(bar, gpu.chipset(), vector.irq_type());
                    gpu.cmdq().drain(bar)?;
                },
