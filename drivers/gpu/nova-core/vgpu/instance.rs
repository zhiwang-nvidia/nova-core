...
pub(crate) struct VgpuInstances<'gpu> {
    /// Declared before `vram` so instance regions are dropped before their allocator.
    instances: KVec<VgpuInstance<'gpu>>,
    vram: Option<VgpuVramAllocator>,
...
impl<'gpu> VgpuInstances<'gpu> {
    ...
    fn alloc_vram_slot(&mut self, mm: &GpuMm<'_>, config: VgpuVramConfig) -> Result<VgpuVramSlot> {
        let replace_allocator = match self.vram.as_ref() {
            Some(allocator) if allocator.is_empty() => !allocator.matches_config(config)?,
            _ => false,
        };
        ...
        if self.vram.is_none() {
            self.vram = Some(VgpuVramAllocator::new(mm, config)?);
        }

        let allocator = self.vram.as_ref().ok_or(EIO)?;
        if !allocator.matches_config(config)? {
            return Err(EBUSY);
        }
        allocator.alloc()
    }

    fn free_vram_slot(&self, slot: VgpuVramSlot) -> Result {
        let allocator = self.vram.as_ref().ok_or(EINVAL)?;
        allocator.free(slot.into_placement())
    }
    ...
    pub(crate) fn allocate_instance(
        ...
        if let Err(error) = ceutils.scrub_guest_fb(dev, cmdq, bar, bar_user, mm, &vram_slot.fbmem) {
            ...
            dev_err!(
                dev,
                "keeping VRAM slot {} in use after scrub error {:?}\n",
                vram_slot.index,
                error,
            );
            return Err(error);
        ...
        let comm = match CommBufferRegion::new(bar_user, mm, &vram_slot.mgmt_heap) {
            ...
            Err(error) => {
                ...
                // A failed BAR1 map can leave some PTEs installed. Without a
                // completed map object they cannot be removed, so keep this
                // placement in use and do not reuse its memory.
                dev_err!(
                    dev,
                    "keeping VRAM slot {} in use after BAR1 error {:?}\n",
                    vram_slot.index,
                    error,
                );
                return Err(error);
...
/// Activate an allocated instance and publish it in the live-instance registry.
///
...
#[expect(clippy::too_many_arguments)]
fn activate_allocated_instance<'gpu>(
...
impl<'gpu> VgpuManager<'gpu> {
    /// Allocate, activate, and publish a vGPU instance.
    ///
    /// Keep the registry locked from allocation through publication or rollback
    /// so duplicate checks, profile limits, and reserved rollback capacity remain
    /// stable throughout activation.
    #[expect(clippy::too_many_arguments)]
    pub(crate) fn create_instance(
        ...
        let instance = instances.allocate_instance(dev, cmdq, bar, bar_user, mm, self, info)?;

        activate_allocated_instance(
            &mut instances,
            dev,
            cmdq,
            bar,
