...
impl GspMsgElement {
    ...
    ///
    /// The GSP echoes `rpc_seq` in the reply, which is what matches that reply to this command.
    pub(crate) fn init(
        rpc_seq: u32,
        cmd_size: usize,
...
/// The MCTP and NVDM transport headers that open every queue element, whatever kind of message
/// follows them.
///
/// Matches `GSP_MSG_QUEUE_ELEMENT` up to its `noEncryption` union member, whose flexible payload
/// array the message header then occupies.
#[repr(C)]
pub(crate) struct QueueElementHeader {
    magic: u32,
    /// Length of the whole element: this header, the message header and the payload.
    element_len: u32,
    mctp: MctpHeader,
    nvdm: NvdmHeader,
    /// Length of the message header and the payload, this header excluded.
    message_len: u32,
    reserved: u32,
}
...
impl QueueElementHeader {
    ...
    /// # Errors
    ///
    /// - `EOVERFLOW` if the resulting element length does not fit the wire fields.
...
/// Flag bit of [`GmcApiHeader::command`] that GSP-RM sets on a response, matching Open RM's
/// `GMCAPI_HEADER_FLAGS_RESPONSE`.
...
impl GspGmcMsgElement {
    ...
    /// Creates an element carrying `payload_size` bytes of GMC command payload.
    ///
...
impl GspArgumentsCached {
    ...
    pub(crate) fn new<'a, 'b>(
        ...
        let (radix3, bindata_size) = bindata
        ...
        let init_inner = init!(bindings::GSP_ARGUMENTS_CACHED {
            ...
            bindataArgs: bindings::GSP_ARGUMENTS_CACHED__bindgen_ty_4 {
                radix3,
                size: bindata_size,
            },
            ..Zeroable::init_zeroed()
...
impl GspArgumentsPadded {
    ...
    pub(crate) fn set_bindata(this: &Coherent<'_, Self>, bindata: Option<&BindataArgs>) {
