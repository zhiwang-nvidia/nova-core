...
/// Send a vGPU command to GSP via GMC and check the response status.
///
/// On success, copies the response payload back into `params`.
fn send_vgpu_command(cmdq: &Cmdq, bar: &Bar0, cmd: u32, params: &mut [u8]) -> Result {
    let response = cmdq.send_gmc_and_receive(bar, cmd, params.as_ref(), 8192)?;
    if response.status != 0 {
        kernel::pr_err!(
            "vGPU command {:#x} failed: status={:#x}\n",
            cmd,
            response.status
        );
        return Err(EIO);
    }
    let copy_len = response.payload.len().min(params.len());
    params[..copy_len].copy_from_slice(&response.payload[..copy_len]);
    Ok(())
}
...
const PLUGIN_BOOT_TIMEOUT: Delta = Delta::from_secs(10);
const PLUGIN_POLL_INTERVAL: Delta = Delta::from_millis(1);
...
const ENGINE_BITMAP_WORDS: usize = NV2080_GPU_MAX_ENGINES.div_ceil(64);
...
impl Vgpu {
    ...
    pub(crate) fn release_chids(&mut self, instance: &mut VgpuInstance) {
        if instance.num_chid > 0 {
            self.chid_alloc
                .free(instance.chid_offset, instance.num_chid);
    ...
    pub(crate) fn wait_plugin_ready(
        ...
        let num_pages = comm_size.div_ceil(page_size) as usize;
    ...
    pub(crate) fn create_instance(
        ...
        if let Err(e) = self.bootload_plugin(cmdq, bar, &instance) {
            self.release_chids(&mut instance);
            return Err(e);
        }
    ...
    pub(crate) fn destroy_instance(&mut self, cmdq: &Cmdq, bar: &Bar0, vf_id: i32) -> Result {
        let pos = self
            .instances
            .iter()
            .position(|inst| inst.id == vf_id)
        ...
        if instance.active {
            ...
            if let Err(e) = self.shutdown_plugin(cmdq, bar, gfid) {
                kernel::pr_warn!("vgpu: shutdown failed for gfid {}: {:?}\n", gfid, e);
            }
            if let Err(e) = self.cleanup_plugin(cmdq, bar, gfid) {
    ...
    pub(crate) fn setup_plugin_rpc(
        ...
        if result != 0 {
            kernel::pr_err!(
                "vgpu: RPC SetupConfigParamsAndInit failed, result_code={:#x}\n",
                result
            );
            return Err(EIO);
        }
...
impl<'a> PluginRpc<'a> {
    fn new(
        ...
        access.try_write32(
            ...
            ctrl_off + ctrl_buf_off::VERSION,
        )?;
        access.try_write64(
            resp_off as u64,
            ctrl_off + ctrl_buf_off::RESPONSE_BUFF_OFFSET,
        )?;
        access.try_write64(msg_off as u64, ctrl_off + ctrl_buf_off::MESSAGE_BUFF_OFFSET)?;
        access.try_write64(
            migration_off as u64,
            ctrl_off + ctrl_buf_off::MIGRATION_BUFF_OFFSET,
        )?;
    ...
    fn send_request(&mut self, bar0: &Bar0, gfid: Gfid, msg_type: RpcMsg, data: &[u8]) -> Result {
        ...
        self.access
            .try_write32(msg_type as u32, self.ctrl_off + ctrl_buf_off::MESSAGE_TYPE)?;
        self.msg_seq_num += 1;
        self.access.try_write32(
            self.msg_seq_num,
            self.ctrl_off + ctrl_buf_off::MESSAGE_SEQ_NUM,
        )?;
    ...
    fn recv_response(&self, data: &mut [u8]) -> Result<u32> {
        ...
        kernel::io::poll::read_poll_timeout(
            || {
                self.access
                    .try_read32(self.resp_off + resp_buf_off::MESSAGE_SEQ_NUM_PROCESSED)
            },
            |val| *val == expected,
            RPC_POLL_INTERVAL,
            RPC_TIMEOUT,
        ...
        let result = self
            .access
            .try_read32(self.resp_off + resp_buf_off::RESULT_CODE)?;
    ...
    fn call(&mut self, bar0: &Bar0, gfid: Gfid, msg_type: RpcMsg, data: &mut [u8]) -> Result {
        self.send_request(bar0, gfid, msg_type, data)?;
        let result = self.recv_response(data)?;
        if result != 0 {
            kernel::pr_err!(
                "vgpu: RPC {:?} failed, result_code={:#x}\n",
                msg_type,
                result
            );
            return Err(EIO);
