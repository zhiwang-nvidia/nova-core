...
impl<'a> ExtCapability<'a> {
    ...
    pub fn cast_sized<U>(self) -> Result<ExtCapability<'a, U>> {
        ...
        // INVARIANT: `self` was a valid capability within extended config space,
        // and we verified `size >= size_of::<U>()` above, so the region is large
        // enough for `U`. The offset is preserved from the original capability.
        Ok(ExtCapability {
            config: self.config,
...
impl ConfigSpace<'_, Extended> {
    ...
    pub fn find_ext_capability(&self, cap: ExtCapId) -> Result<ExtCapability<'_>> {
        let cap_id = i32::from(cap.as_raw());
        // SAFETY: `self.pdev` is valid by the type invariant of `ConfigSpace`.
        let offset =
            usize::from(unsafe { bindings::pci_find_ext_capability(self.pdev.as_raw(), cap_id) });
    ...
    pub fn find_next_ext_capability(&self, start: u16, cap: ExtCapId) -> Result<ExtCapability<'_>> {
        let cap_id = i32::from(cap.as_raw());
        // SAFETY: `self.pdev` is valid by the type invariant of `ConfigSpace`.
        let offset = usize::from(unsafe {
            bindings::pci_find_next_ext_capability(self.pdev.as_raw(), start, cap_id)
        });
    ...
    fn make_ext_capability(&self, offset: usize) -> ExtCapability<'_> {
        ...
        // INVARIANT: `offset` came from `pci_find_ext_capability` /
        // `pci_find_next_ext_capability`, which return valid capability offsets
        // within extended config space. `size` covers from `offset` to the next
        // capability (or end of config space).
