...
impl ExtCapability<'_, ExtSriovRegs> {
    ...
    // CAST: Sign-extend the 32-bit mask to 64 bits for 64-bit BAR addresses.
    // `as` is used only in this compile-time constant.
    const MEM_BAR_ADDR_MASK_64: u64 = bindings::PCI_BASE_ADDRESS_MEM_MASK as i64 as u64;

    /// Returns whether the VF BAR at `bar_index` is a 64-bit memory BAR.
    pub fn vf_bar_is_64bit(&self, bar_index: usize) -> Result<bool> {
        let raw = crate::io_read!(self, .vf_bar[bar_index]?);
        Ok(raw & bindings::PCI_BASE_ADDRESS_MEM_TYPE_MASK == bindings::PCI_BASE_ADDRESS_MEM_TYPE_64)
    }

    /// Reads a 64-bit VF BAR base address, masking out the type/control bits.
    pub fn read_vf_bar64_addr(&self, bar_index: usize) -> Result<u64> {
        Ok(self.read_vf_bar64(bar_index)? & Self::MEM_BAR_ADDR_MASK_64)
    }
}
