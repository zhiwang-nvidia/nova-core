...
``CONFIG_NOVA_CORE_SELFTESTS``, which also gates the other probe-time hardware
tests. It runs after the GPU's own firmware has booted and before GSP boot, so
it never touches GSP interrupt state.

A failed interrupt delivery test fails the probe, unlike the other tests that
option gates, which only report their failures. An interrupt path that does not
work leaves the driver unable to make progress later, in a place that says
...
The doorbell vector is hardwired rather than assigned by GSP-RM, which is what
lets the test run before GSP-RM is up. The test allocates the vectors for the
doorbell's own subtree and releases them before returning, so the driver's own
allocation covers the GSP subtree and nothing else. Sharing one allocation would
have made the test pass on the coincidence that both vectors land in subtree 2.
